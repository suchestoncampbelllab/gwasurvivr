...
head(pheno.file)
```
Categorical variables need to be converted into indicator (dummy) variables and be of class `numeric`. In this example, we will select samples from the `experimental` group will test survival only on these genotypes. The first column in the `pheno.file` needs to be sample IDs that we want to match on. 

We use `tidyverse` and `magrittr` packages in this example solely for data preparation/manipulation purposes.
...
sangerCoxSurv(vcf.file=vcf.file,
              ...
              output.name=tempfile("sanger_example"),
              chunk.size=10000,
              info.filter=0.7,
              maf.filter=0.01,
              verbose=TRUE)
...
```{r, message=FALSE}
sanger_res <- read_tsv(dir(tempdir(), pattern="^sanger_example.*\\.coxph$", full.names = TRUE))
```
...
Now we can run survival. Need to talk about why `flip.dosage=TRUE`
```{r, echo=FALSE}
impute2CoxSurv(impute.file=impute.file,
               ...
               inter.term=NULL,
               print.covs="only",
               out.file=tempfile("impute_example"),
               chunk.size=10000,
               maf.filter=0.01,
               info.filter=0.7,
               flip.dosage=TRUE,
...
impute2_res <- read_tsv(dir(tempdir(), pattern="^impute_example.*\\.coxph$", full.names=TRUE))
```
