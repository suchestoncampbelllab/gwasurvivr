...
```{r, message=FALSE, eval=FALSE}
sanger_intx_only <- read.table("sanger_intx_only.coxph", sep="\t", header=TRUE, stringsAsFactors = FALSE)
```

```{r, message=FALSE, echo=FALSE}
sanger_intx_only <- read.table(dir(tempdir(), pattern="^sanger_intx_only.*\\.coxph$", full.names = TRUE), sep="\t", header=TRUE, stringsAsFactors = FALSE)
```

```{r}
str(head(sanger_intx_only))
```
