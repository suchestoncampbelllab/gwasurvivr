createGdsCoxSurv <- function(gdsfile,
    ...
    cox_surv <- list(
        ...
        snp.df = data.frame(matrix(
            data = rep(NA, 16), ncol = 8
        )),
...
processSNPGenotypes.GdsCoxSurv <-
    function(x,
             snp,
             genotypes,
             scanAnn,
        ...
        genotypes <- genotypes[, scanAnn$ID_2 %in% cox.params$id]
...
getGenoData.GdsCoxSurv <- function(x) {
    ...
    # aux files
    snpfile <-
        replaceFileExt(file.path = x$gdsfile, ext = ".snp.rdata")
    scanfile <-
        replaceFileExt(file.path = x$gdsfile, ext = ".scan.rdata")
    # read in snp data
