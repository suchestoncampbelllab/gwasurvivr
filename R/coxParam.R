...
.coxParam <- function(pheno.file, time.to.event, event, covariates, sample.ids){
        ...
        params <- list(pheno.file=pheno.file[,covariates], Y=Y, STRATA=STRATA, OFFSET=OFFSET, CONTROL=CONTROL, WEIGHTS=WEIGHTS, METHOD=METHOD, ROWNAMES=ROWNAMES, INIT=INIT)
        return(params)
}
