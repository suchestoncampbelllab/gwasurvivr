...
#'  from: \code{"DS"} (ALT-allele dosage, the default, as produced by the
#'  Michigan/Minimac imputation server), \code{"GT"} (hard genotype calls,
#'  converted to ALT-allele counts of 0/1/2), or \code{"HDS"} (per-haplotype
#'  dosages, summed to a diploid ALT dosage). Use \code{"GT"} for VCFs that
#'  carry only hard calls.
...
#'
#' Effect allele:
#' gwasurvivr models additive dosage of the \strong{ALT} allele. The reported
#' hazard ratio (\code{HR}) is therefore the multiplicative change in hazard per
#' one additional copy of the ALT allele. To remove any ambiguity, the output
#' includes an \code{EFFECT_ALLELE} column (always the ALT allele here) and an
#' \code{OTHER_ALLELE} column (the REF allele). \code{HR > 1} means the ALT
#' allele is associated with higher hazard (worse survival); \code{HR < 1} means
#' it is protective. Note "minor" vs "major" is sample-dependent: the minor
#' allele is simply whichever of REF/ALT is less frequent in your samples
#' (\code{SAMP_MAF} reports that minor-allele frequency), and it may be either
#' the effect or the other allele.
#
#' @return
...
#' Output columns include \code{EFFECT_ALLELE} (= ALT, the allele the HR refers
#' to) and \code{OTHER_ALLELE} (= REF), alongside \code{COEF}, \code{SE.COEF},
#' \code{HR}, its 95\% CI, \code{Z}, \code{PVALUE}, and sample/event counts.
#'
#' @examples 
